...
## Magma
```Magma
tr:=func<m|IsSquare(8*m+1)>; pal:=func<n|Intseq(n) eq Reverse(Intseq(n))>; f:=func<n |n le 1 select 0 else n*(&+[Factorisation(n)[i][2] / Factorisation(n)[i][1]: i in [1..#Factorisation(n)]])>; [n:n in [d*(d+1) div 2:d in [0..150000]]| pal(Floor(f(n)))];
```
## Python
```Python
