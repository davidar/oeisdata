...
## Magma
```Magma
f3:=func<n|forall{i:i in [1..#Intseq(n) by 2]| Intseq(n)[i] eq 3}>;  fc:=func<n|  forall{i:i in [2..#Intseq(n) by 2]| Intseq(n)[i] ne 3}>; [p:p in PrimesUpTo(40000)|f3(p) and fc(p)]; // _Marius A. Burtea_, Oct 22 2021
```
## Python
```Python
