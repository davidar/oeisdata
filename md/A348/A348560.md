...
## Magma
```Magma
f7:=func<n|forall{i:i in [1..#Intseq(n) by 2]| Intseq(n)[i] eq 7}>;  fc:=func<n|  forall{i:i in [2..#Intseq(n) by 2]| Intseq(n)[i] ne 7}>; [p:p in PrimesUpTo(80000)|f7(p) and fc(p)]; // _Marius A. Burtea_, Oct 22 2021
```
## Python
```Python
