...
```
```PARI
a(n)=my(s); forsquarefree(k=1,n, s+=moebius(k)*(n\k[1])^3); s \\ _Charles R Greathouse IV_, Jan 08 2018
```
```PARI
my(N=66, x='x+O('x^N)); Vec(sum(k=1, N, moebius(k)*x^k*(1+4*x^k+x^(2*k))/(1-x^k)^3)/(1-x)) \\ _Seiichi Manyama_, May 22 2021
```
## Python
```Python
