...
## PARI
```PARI
list(lim)=my(v=List(),N);for(n=0,log(lim)\log(7),N=7^n;while(N<=lim,listput(v,N);N*=5));vecsort(Vec(v)) \\ _Charles R Greathouse IV_, Jun 28 2011
```
## Magma
```Magma
[n: n in [1..600000] | PrimeDivisors(n) subset [5,7]]; // _Bruno Berselli_, Sep 24 2012
```
## Haskell
```Haskell
