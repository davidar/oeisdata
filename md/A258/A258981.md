...
## Sage
```Sage
[0]+[n for n in [1..1000000] if max(n.digits(base=3))==1 and max(n.digits(base=4))==1] # _Tom Edgar_, Jul 11 2015
```
## PARI
```PARI
...
```
```PARI
isok(n) = (n==0) || ((vecmax(digits(n,3)) < 2) && (vecmax(digits(n,4)) < 2)); \\ _Michel Marcus_, Aug 05 2015
```
```PARI
print1(0);for(n=1,1e5,vecmax(digits(t=subst(Pol(binary(n)),'x,4),3))<2&&print1(","t)) \\ _M. F. Hasler_, Feb 01 2016
```
## Python
```Python
