# Write n in base 5 and juxtapose\.
https://oeis.org/A031219
## JSON
```JSON
[1, 2, 3, 4, 1, 0, 1, 1, 1, 2, 1, 3, 1, 4, 2, 0, 2, 1, 2, 2, 2, 3, 2, 4, 3, 0, 3, 1, 3, 2, 3, 3, 3, 4, 4, 0, 4, 1, 4, 2, 4, 3, 4, 4, 1, 0, 0, 1, 0, 1, 1, 0, 2, 1, 0, 3, 1, 0, 4, 1, 1, 0, 1, 1, 1, 1, 1, 2, 1, 1, 3, 1, 1, 4, 1, 2, 0, 1, 2, 1, 1, 2, 2, 1, 2, 3, 1, 2, 4, 1]
```
## Mathematica
```Mathematica
Flatten@ IntegerDigits[ Range@ 40, 5] (* or *)
```
```Mathematica
almostNatural[n_, b_] := Block[{m = 0, d = n, i = 1, l, p}, While[m <= d, l = m; m = (b - 1) i*b^(i - 1) + l; i++]; i--; p = Mod[d - l, i]; q = Floor[(d - l)/i] + b^(i - 1); If[p != 0, IntegerDigits[q, b][[p]], Mod[q - 1, b]]]; Array[ a[#, 5] &, 105] (* _Robert G. Wilson v_, Jul 01 2014 *)
```
## Magma
```Magma
&cat[Reverse(IntegerToSequence(n,5)):n in[1..31]]; // _Jason Kimberley_, Dec 07 2012
```
## Python
```Python
from itertools import count, chain, islice
from sympy.ntheory.factor_ import digits
def A031219_gen(): return chain.from_iterable(digits(m, 5)[1:] for m in count(1))
A031219_list = list(islice(A031219_gen(), 30)) # _Chai Wah Wu_, Jan 07 2022
```
