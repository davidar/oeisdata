...
```
```Mathematica
Select[Range[100],Max[FactorInteger[#][[All,1]]]<20&] (* _Harvey P. Dale_, Sep 20 2018 *)
```
## PARI
```PARI
...
```
```PARI
list(lim,p=19)=if(p==2, return(powers(2, logint(lim\1,2)))); my(v=[],q=precprime(p-1),t=1); for(e=0,logint(lim\=1,p), v=concat(v, list(lim\t,q)*t); t*=p); Set(v) \\ _Charles R Greathouse IV_, Apr 16 2020
```
## Magma
```Magma
[n: n in [1..100] | PrimeDivisors(n) subset PrimesUpTo(19)]; // _Bruno Berselli_, Sep 24 2012
```
## Python
```Python
import heapq
