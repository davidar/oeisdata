...
select(q, [$0..2200])[];  # _Alois P. Heinz_, Dec 30 2023
```
## Mathematica
```Mathematica
Select[Range[0, 2200], Total[(d = IntegerDigits[#])^3] == Total[d]^2 &] (* _Amiram Eldar_, Dec 30 2023 *)
```
## Python
```Python
...
## PARI
```PARI
isok(k) = my(d=digits(k)); vecsum(d)^2 == vecsum(apply(x->x^3, d)); \\ _Michel Marcus_, Dec 30 2023
```
