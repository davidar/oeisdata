...
## Magma
```Magma
[p: p in PrimesUpTo(10000) | &+Intseq(p) eq 10]; // _Vincenzo Librandi_, Jul 08 2014
```
## PARI
```PARI
forprime(p=19,8101,if(10==sumdigits(p),print(p","))) \\ _Zak Seidov_, Oct 08 2016
```
```PARI
(A107579_nxt(p)=until(isprime(p=A228915(p)),); p); A107579_first(N=100)=vector(N, i, p=if(i>1, A107579_nxt(p), 19))) \\ _M. F. Hasler_, Mar 15 2022
```
## Python
```Python
