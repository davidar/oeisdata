...
## Mathematica
```Mathematica
Select[Range@20000,Total[(a=IntegerDigits@#)^Range[0,Length@a-1]]==Total[a!]&] (* _Giorgos Kalogeropoulos_, Mar 30 2021 *)
```
## Python
```Python
...
## PARI
```PARI
is(n) = my(d = digits(n)); sum(i = 1, #d, d[i]!) == sum(i = 1, #d, d[i]^(i-1)) \\ _David A. Corneth_, Mar 30 2021
```
