...
```
```PARI
is(n)=my(f=factor(n),t=#f~); (t>1 || (t==1 && f[1,2]>1)) && hammingweight(n)==sum(i=1,t, hammingweight(f[i,1])*f[i,2]) \\ _Charles R Greathouse IV_, Dec 02 2016
```
## Python
```Python
