...
## PARI
```PARI
is(n)=my(v=vecsort(eval(Vec(Str(n))),,8)); if(v[1]==0, return(0)); for(i=1, #v, if(n%v[i], return(0))); 1 \\ _Charles R Greathouse IV_, Apr 17 2012
```
```PARI
is_A034838(n)=my(d=Set(digits(n)));d[1]&&!forstep(i=#d,1,-1,n%d[i]&&return) \\ _M. F. Hasler_, Jan 10 2016
```
## Python
```Python
A034838_list = []
...
## Magma
```Magma
[n:n in [1..500]| not 0 in Intseq(n) and #[c:c in [1..#Intseq(n)]| n mod Intseq(n)[c] eq 0] eq #Intseq(n)] // _Marius A. Burtea_, Sep 12 2019
```
