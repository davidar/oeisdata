...
## PARI
```PARI
isok(n) = {my(v = vecsort(digits(n))); my(d = -1); for(i=1, #v, if (v[i] == d, rl++; if (rl > 2, return (0)); , d = v[i]; rl = 1);); return (1);} \\ _Michel Marcus_, Sep 29 2015
```
```PARI
is(n)=my(d=vecsort(digits(n))); for(i=3,#d, if(d[i]==d[i-2], return(0))); 1 \\ _Charles R Greathouse IV_, Oct 20 2022
```
## Python
```Python
